	...
	if ($ipx=~/\//){
		...
		if (!$range){
			...
			#	print lres "$ipf;$iporta;$nome_rede;$origem_rede;$ipx;\n";
				if ($hash_ips{$ipf}{$iporta}){
							
					$hash_ips{$ipf}{$iporta}.=",".$nome_rede;
...
##
                   #print "$ipt ||| $i || range $range ||| $ipx\n";
					foreach $iporta (@portas){
						...
						if ($hash_ips{$ipt}{$iporta}){
							
							$hash_ips{$ipt}{$iporta}.=",".$nome_rede;
...
#####		
		...
		foreach $iporta (@portas){
			...
			if ($hash_ips{$ipx}{$iporta}){
				$hash_ips{$ipx}{$iporta}.=",".$nome_rede;
			}
			else{
